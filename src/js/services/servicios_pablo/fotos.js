document.addEventListener("DOMContentLoaded", async function() {
    ...
    // Aquí ya tienes tus fotos reales
    const fotos = datosPreguntas.fotos.map(f => ({
        src: f.foto,
        opciones: [] // luego añadimos las respuestas
    }));

    // ahora pedimos las respuestas
    const resultadoRespuestas = await fetch("https://22.daw.esvirgua.com/app/html/Adrian/src/src/php/indexAdrian.php?c=C_ListarFotos&m=mostrarRespuestasFotos");
    const datosRespuestas = await resultadoRespuestas.json();

    console.log("Respuestas desde PHP:", datosRespuestas);

    // Montar las respuestas dentro de cada foto
    fotos.forEach((foto, i) => {
        foto.opciones = datosRespuestas.respuestasFotos
            .filter(r => r.idFoto == foto.idFoto)
            .map(r => ({
                texto: r.respuestaFoto,
